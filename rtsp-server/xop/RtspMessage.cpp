...
bool RtspRequest::ParseRequestLine(const char* begin, const char* end)
{
	...
	request_line_param_.emplace("url_port", make_pair("", static_cast<uint32_t>(port)));
...
{
	if (const std::size_t pos = message.find("CSeq"); pos != std::string::npos) {
		uint32_t cseq = 0;
...
{
	if(const auto iter = request_line_param_.find("url"); iter != request_line_param_.end()) {
		...
		if (pos != std::string::npos) {
			int channel_index;
                        ...
                        channel_id_ = static_cast<MediaChannelId>(channel_index);
                        return true;
		}       
	}
	return false;
...
{
	uint32_t cseq = 0;
	if(const auto iter = header_line_param_.find("cseq"); iter != header_line_param_.end()) {
...
{
	if(const auto iter = request_line_param_.find("url_ip"); iter != request_line_param_.end()) {
...
{
	if(const auto iter = request_line_param_.find("url_suffix"); iter != request_line_param_.end()) {
		return iter->second.first;
	}
...
{
	if(const auto iter = header_line_param_.find("rtp_channel"); iter != header_line_param_.end()) {
...
{
	if(const auto iter = header_line_param_.find("rtcp_channel"); iter != header_line_param_.end()) {
		return iter->second.second;
	}
...
{
	if(const auto iter = header_line_param_.find("rtp_port"); iter != header_line_param_.end()) {
		return iter->second.second;
	}
...
{
	if(const auto iter = header_line_param_.find("rtcp_port"); iter != header_line_param_.end()) {
		return iter->second.second;
	}
...
int RtspRequest::BuildDescribeRes(const char* buf, const int buf_size, const char* sdp) const
{
	memset((void*)buf, 0, buf_size); //TODO
	snprintf(const_cast<char *>(buf), buf_size,
			"RTSP/1.0 200 OK\r\n"
...
int RtspRequest::BuildSetupMulticastRes(const char* buf, const int buf_size, const char* multicast_ip, const uint16_t port, const uint32_t session_id) const
{	
	memset((void*)buf, 0, buf_size); //TODO
	snprintf(const_cast<char *>(buf), buf_size,
			"RTSP/1.0 200 OK\r\n"
...
int RtspRequest::BuildSetupUdpRes(const char* buf, const int buf_size, const uint16_t rtp_chn, const uint16_t rtcp_chn, const uint32_t session_id) const
{
	memset((void*)buf, 0, buf_size); //TODO
	snprintf(const_cast<char *>(buf), buf_size,
			"RTSP/1.0 200 OK\r\n"
...
int RtspRequest::BuildSetupTcpRes(const char* buf, const int buf_size, const uint16_t rtp_chn, const uint16_t rtcp_chn, const uint32_t session_id) const
{
	memset((void*)buf, 0, buf_size); //TODO
	snprintf(const_cast<char *>(buf), buf_size,
			"RTSP/1.0 200 OK\r\n"
...
int RtspRequest::BuildPlayRes(const char* buf, const int buf_size, const char* rtp_info, const uint32_t session_id) const
{
	memset((void*)buf, 0, buf_size); //TODO
	snprintf(const_cast<char *>(buf), buf_size,
			"RTSP/1.0 200 OK\r\n"
	...
	if (rtp_info != nullptr) {
		snprintf(const_cast<char *>(buf) + strlen(buf), buf_size - strlen(buf), "%s\r\n", rtp_info);
	}
...
int RtspRequest::BuildTeardownRes(const char* buf, const int buf_size, const uint32_t session_id) const
{
	memset((void*)buf, 0, buf_size); //TODO
	snprintf(const_cast<char *>(buf), buf_size,
			"RTSP/1.0 200 OK\r\n"
...
int RtspRequest::BuildGetParamterRes(const char* buf, const int buf_size, const uint32_t session_id) const
{
	memset((void*)buf, 0, buf_size); //TODO
	snprintf(const_cast<char *>(buf), buf_size,
			"RTSP/1.0 200 OK\r\n"
...
int RtspRequest::BuildUnauthorizedRes(const char* buf, const int buf_size, const char* realm, const char* nonce) const
{
	memset((void*)buf, 0, buf_size); //TODO
	snprintf(const_cast<char *>(buf), buf_size,
...
bool RtspResponse::ParseResponse(BufferReader *buffer)
{
	if (strstr(buffer->Peek(), "\r\n\r\n") != nullptr) {
		...
		if (char* ptr = strstr(buffer->Peek(), "Session"); ptr != nullptr) {
			if (char session_id[50] = {0}; sscanf(ptr, "%*[^:]: %s", session_id) == 1)
				session_ = session_id;
		}
...
int RtspResponse::BuildAnnounceReq(const char* buf, const int buf_size, const char *sdp)
{
	memset((void*)buf, 0, buf_size); //TODO
	snprintf(const_cast<char *>(buf), buf_size,
			"ANNOUNCE %s RTSP/1.0\r\n"
...
int RtspResponse::BuildSetupTcpReq(const char* buf, const int buf_size, const int trackId)
{
